...
void ofApp::update() {
	...
	float threshold_1 = 0.5 - 0.02;
	float threshold_2 = 0.5 + 0.02;

	for (float threshold_base = 0; threshold_base <= 1.0; threshold_base += 0.1)
	{
		float threshold_1 = threshold_base - 0.01;
		float threshold_2 = threshold_base + 0.01;

		for (float phi_deg = 0; phi_deg < 360; phi_deg += phi_deg_step) {

			for (float theta_deg = 0; theta_deg < 360; theta_deg += theta_deg_step) {

				auto noise_location = this->make_point(R, r, theta_deg, phi_deg) * 0.003;
				auto noise_value = ofNoise(noise_location.x, noise_location.y, ofGetFrameNum() * 0.0025, noise_location.z);
